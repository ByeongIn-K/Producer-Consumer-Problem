...
Condition Variable 방식과 Semaphore 방식 모두 Race Condition을 제거하고 x를 항상 0~30 범위 내에서 유지할 수 있었다.

실험 결과 본 환경(Ubuntu 20.04, GCC 9.4.0)에서는 Semaphore 방식이 Condition Variable 방식보다 약간 더 빠른 실행 시간을 보였다.

이는 Semaphore가 상대적으로 단순한 동기화 구조를 사용하기 때문으로 판단된다.

또한 Consumer와 Producer의 스레드 수를 증가시켜 추가 실험을 수행한 결과에서도 Semaphore 방식이 전반적으로 더 빠른 실행 시간을 보이는 경향을 확인할 수 있었다.

다만 두 방법 모두 운영체제의 스케줄링 정책, CPU 환경, 시스템 부하 등에 영향을 받을 수 있으므로 다른 환경에서는 결과가 달라질 수 있다.
...
성능 측정 결과 본 실험 환경에서는 Semaphore 방식이 약간 더 좋은 성능을 보였지만, 두 방식 모두 올바르게 동작하며 운영체제 환경에 따라 결과가 달라질 수 있음을 확인하였다.
